<template>
    ...
    <div class="p-4">
        ...
        <div class="flex justify-end">
                <p class="text-lg">Count: {{ roles.length }}</p>
        </div>
        ...
        <p v-else class="mt-4">No Roles Yet...</p>
    </div>
</template>
...
const deleteRole = function (role_id) {
    Swal("Delete Role Access!", "Are you sure?", "warning", "Yes", {
        ...
        console.log(role_id,'WTF');
        if (result.value) {
            const access_token = localStorage.getItem("bearer_token");
            axios
                .delete(`/api/role-access/${role_id}`, {
                    headers: {
                        Authorization: `Bearer ${access_token}`,
                    },
                })
                .then((response) => {
                    ...
                    showCustomToast("success", "Role Access Deleted!");
