<template>
    <Navbar/>
    <div class="p-4">
        <h1 class="text-2xl font-bold text-center">USER ROLES</h1>
        ...
        <div class="flex justify-end">
                <p class="text-lg">Count: {{ user_roles.length }}</p>
        </div>
        <template v-if="user_roles.length">
            <div
            ...
            >
                <div
                    ...
                    :key="role.id"
                    class="flex flex-col justify-between p-4 border rounded-md border-zinc-400"
                >
                    <div>
                        <div class="text-center">
                            <p class="mt-2">
                                <span class="text-lg">Name: </span>{{ role.name }}
                            </p>
                        </div>
                    </div>
        ...
        <p v-else class="mt-4">No UserRoles Yet...</p>
    </div>
</template>
...
const getUserRoles = () => {
    ...
    axios
        ...
        .then((response) => {
            ...
            user_roles.value = response.data;
        });
};


const deleteUserRole = function (role_id) {
    Swal("Delete UserRole!", "Are you sure?", "warning", "Yes", {
        ...
        if (result.value) {
            ...
            axios
                ...
                .then((response) => {
                    // Remove the deleted role from the user_roles array
                    user_roles.value = user_roles.value.filter(
                        (role) => role.id !== role_id
                    );
                    showCustomToast("success", "UserRole Deleted!");
                    // router.push({ name: "user_roles" });
                    // console.log(response);
                })
                .catch((error) => {
                    console.log(error);
