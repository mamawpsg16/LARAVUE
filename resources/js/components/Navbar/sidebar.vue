...
const getModules = async function(){
    axios.get('/api/getModules',{headers: {
                    Authorization: `Bearer ${getItem("access_token")}`,
                }})
    .then((response) =>{
        const { modules, permissions } = response.data;
        const userModules = permissions?.map(permission => permission.module_id).filter(moduleId => modules.some(module => module.id === moduleId));
        const assigned_modules  = modules.filter(module => userModules.includes(module.id))
        links.value = assigned_modules;
